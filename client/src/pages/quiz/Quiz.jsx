...
const Quiz = () => {
  ...
  const updateScore = async (totalScore) => {
    try {
      const updateScoreResp = await Axios.post(
        APP_SERVER + "/api/quiz/" + quizId,
        { totalScore },
        {
          headers: {
            Authorization: "Bearer " + Cookies.get("token"),
    ...
    return (
      <div className="flex flex-col items-center">
        ...
        <p className="mb-8 text-2xl">
          Your Score: <span className="text-cteal">{score}/5</span>
        </p>
        ...
        <ul className="self-start list-disc ml-4">
          {questions?.map((question, index) => (
            <li key={index} className="mb-1">
              {question.question}{" "}
              <p className="text-cteal">{question.options[question.answer]}</p>
            </li>
          ))}
        </ul>
        <button
